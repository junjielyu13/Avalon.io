...
export class ApiService {
  ...
  async createPlayer(data: Prisma.PlayerCreateInput): Promise<any> {
    // 获取当前的玩家code
    const playerUtil = await this.prisma.util.findFirst();
    let playerCount = 0;
    // 如果没有没找到util则创建之
    if (!playerUtil) {
    ...
    // 尝试创建玩家
    const ret = await this.prisma.player.create({ data });
    // 创建玩家失败
    if (!ret) {
  ...
  async leaveRoom(player_code: string): Promise<any> {
    ...
    if (!player.room_id) {
      ...
      return {
        code: '400',
        msg:
          'Player ' + player_code + 'is not joined the room, leave room failed',
      };
    }
    const room = await this.prisma.room.findUnique({
  ...
  async createRoomByPlayerID(
    player_id: string,
    player_name: string = null,
  ): Promise<any> {
    // 获取当前的房间
    const roomUtil = await this.prisma.util.findFirst();
    let roomCount = 0;
    // 如果没有没找到util则创建之
    if (!roomUtil) {
      await this.prisma.util.create({
    ...
    // 尝试创建房间
    const ret = await this.prisma.room.create({
      data: {
